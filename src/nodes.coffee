...
exports.Pattern = class Pattern extends Base
  ...
  compileNode: (o) ->
    return @matches[0].compile(o) if @matches.length is 1

    ifblock = null
    lastif = null
    for m in @matches.reverse()
      conditions = "arguments.length === #{m.params.length}"
      for p, i in m.params
        if p.name.value is '_'
          conditions += ""
        else if !isNaN(p.name.value) or /'(.*)'/.test p.name.value
          conditions += " && arguments[#{i}] === #{p.name.value}"
        else
          m.body.expressions = [ new Assign(p.name, new Literal("arguments[#{i}]")) ].concat m.body.expressions
      ifblock = new If(new Literal(conditions), m.body)
      ifblock.addElse(lastif) if lastif
      lastif = ifblock

    new Code([], new Block [ ifblock ], 'func').compile(o)
...
exports.LeftArrow = class LeftArrow extends Base
  ...
  compileNode: (o) ->
    ...
    val = "return " + @value.compile o, LEVEL_LIST
    val
